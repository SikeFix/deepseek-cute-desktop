...
下载即用的本地 DeepSeek 桌面端：官方 DeepSeek 桌面客户端体验：本地运行、官方会话界面、历史记录、模型服务设置、诊断信息与可控更新。macOS 和 Windows 都不需要用户额外安装 Node.js 或手动运行 `dsh web`，启动时也不会打开系统浏览器。
...
```text
767f7f7585b945194fb60b554ccaff92b00c16ae722ad09cbd3e054027197c2a  DeepSeek-M2-1.8.0.dmg
27d8cc88f8e5a8cb4344067b14418585d8ae4e9abbd1b5af3a0891af9c97b047  DeepSeek-Cute-Windows-x64-Setup-1.8.0.exe
```
...
- macOS 1.8.0 已在 M2 环境完成编译、ad-hoc 签名、DMG 挂载与内置 0.1.5-rc.2 运行时验证（从 DMG 镜像直接启动，本地服务 200）。
- Windows 1.8.0 由 GitHub Actions 在 windows-latest 完成 electron-builder x64 构建，打包后逐文件自检关键运行时（缺失即构建失败），并核对 NSIS 更新元数据与差分 blockmap。
